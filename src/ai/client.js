  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (status === 413 || errMsg.includes('413') || errMsg.includes('Request too large') || errMsg.includes('ITPM') || errMsg.includes('reduce your message size')) {
        const fallback = config.ai.fallbackModel || (config.ai.provider === 'groq' ? 'groq/compound-mini' : 'openrouter/free');
      ...
      if (isRetryable) {
        ...
        const fallbackModel = config.ai.fallbackModel || (config.ai.provider === 'groq' ? 'groq/compound-mini' : 'openrouter/free');
